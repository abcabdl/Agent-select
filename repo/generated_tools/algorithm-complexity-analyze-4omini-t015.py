...
def _post_process_algorithm(code):
    """
    algorithm特定的后处理逻辑
    """
    ...
    # 算法复杂度检查
    if "# O(" not in code and "# Time:" not in code:
        # 添加复杂度注释提示
        code = "    # Time: O(?)  Space: O(?)\n" + code
...
def _call_llm(prompt):
    ...
    system = """You are an algorithm optimization expert. Design efficient solutions with complexity analysis.
CRITICAL RULES:
...
5. Analyze time/space complexity"""
    ...
    user = """Design algorithm for: {prompt}. Analyze complexity. Use exact parameter names. Return body only."""
