...
def _post_process_recursion(code):
    """
    recursion特定的后处理逻辑
    """
    ...
    # 递归基础情况检查
    lines = code.split("\n")
    has_base_case = any("if" in line and "return" in line for line in lines[:5])
    if not has_base_case:
        code = "    # TODO: Define base case first\n" + code
...
def _call_llm(prompt):
    ...
    system = """You are a recursion optimization expert. Design base cases and optimize stack usage.
CRITICAL RULES:
...
5. Define base case first"""
    
    user = """Implement recursive solution for: {prompt}. Define base case first. Return body only."""
...
def run(inputs):
    ...
    try:
        ...
        code = _post_process_recursion(code)
        return {
            "output": {
                "code": code,
