...
def run(inputs):
    """
    Tool for code refactoring restructure15 implement1
    专注于: restructure15 operations
    """
    task = inputs.get("task") or inputs.get("query") or inputs.get("prompt") or ""
    mode = "restructure15"
    
    if not task or not isinstance(task, str):
        return {"output": {"error": "No valid task provided", "mode": mode}}
    ...
    result = {
        ...
        "specialty": "code",
        "action": "implement1",
        "output": f"Processed {{task}} with {{mode}} strategy"
    }
