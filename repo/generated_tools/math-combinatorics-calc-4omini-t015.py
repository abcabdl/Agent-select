...
def _post_process_math(code):
    ...
    has_overflow_check = "try:" in code or "except" in code
    # 提示大数处理
    if "**" in code or "*" in code:
        if not (has_modulo or has_overflow_check):
            code = "    # TODO: Consider overflow and modulo\n" + code
...
def _call_llm(prompt):
    ...
    system = """You are a mathematical computation expert. Handle numerical precision and overflow.
CRITICAL RULES:
...
5. Handle overflow and precision"""
    
    user = """Solve mathematically: {prompt}. Handle overflow and precision. Return body only."""
