...
def _post_process_robustness(code):
    """
    robustness特定的后处理逻辑
    """
...
def _call_llm(prompt):
    ...
    system = """You are an error handling specialist. Implement defensive programming.
CRITICAL RULES:
...
5. Write clean, correct code"""
    ...
    user = """Add error handling for: {prompt}. Use try/except. Provide fallbacks. Return body only."""
...
def run(inputs):
    ...
    try:
        ...
        code = _post_process_robustness(code)
        return {
            "output": {
                "code": code,
