"""
string-format-template-4omini-t37: Expert in string manipulation: Unicode, parsing, formatting, escape sequences, pattern matching
"""
import os
import httpx
import json
...
def _post_process_string(code: str) -> str:
    """字符串专用后处理:检查Unicode和编码"""
    # 确保有encode/decode考虑
    if 'unicode' in code.lower() or 'utf' in code.lower():
        if 'encode' not in code and 'decode' not in code:
            code = '# Note: Consider encoding/decoding\n' + code
    return code
...
def run(inputs):
    ...
    system_prompt = 'You are an expert in string manipulation. Generate clean, efficient Python code for string operations including Unicode handling, parsing, formatting, and pattern matching. Focus on edge cases and character encoding.'
    ...
    # 后处理
    code = _post_process_string(code)
