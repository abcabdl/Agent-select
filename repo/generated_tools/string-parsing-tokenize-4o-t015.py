...
def _post_process_string(code):
    ...
    # 字符串特定检查
    lines = code.split("\n")
    has_encode_decode = any("encode" in line or "decode" in line for line in lines)
    has_strip = any("strip" in line for line in lines)
    # 如果处理字符串但没有基本处理,添加注释提示
    if "str" in code and not (has_encode_decode or has_strip):
        code = "    # TODO: Consider str.strip() and encoding handling\n" + code
...
def _call_llm(prompt):
    ...
    system = """You are a string manipulation specialist. Focus on Unicode handling, escape sequences, and edge cases.
CRITICAL RULES:
...
5. Handle Unicode and empty strings"""
    
    user = """Task: {prompt}. Handle Unicode, empty strings, and special characters. Return function body only."""
