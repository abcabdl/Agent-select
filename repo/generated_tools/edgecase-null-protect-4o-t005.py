...
def _post_process_edgecase(code):
    """
    edgecase特定的后处理逻辑
    """
    ...
    has_empty_check = "not " in code or "len(" in code
    if not (has_none_check or has_empty_check):
        code = "    # TODO: Add None/empty checks\n" + code
...
def _call_llm(prompt):
    ...
    system = """You are an edge case validation expert. Check boundaries and invalid inputs.
CRITICAL RULES:
...
5. Check all boundary conditions"""
    ...
    user = """Handle edge cases for: {prompt}. Check None, empty, overflow. Return body only."""
